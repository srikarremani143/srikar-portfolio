export const personalInfo = {
  ...
  title: "Applied Gen AI Engineer & Code Judge",
...
export const projects: Project[] = [
  ...
  },
  {
    ...
    highlights: [
      "Built an end-to-end AI-native cricket intelligence stack: FastAPI backend and Next.js (TypeScript, Tailwind, Recharts) frontend, querying a multi-format Google BigQuery warehouse (IPL/BBL/MLC ball-by-ball, ODI/T20 career tables, Test matches, partnerships, venues).",
      "Implemented natural language to SQL: schema-grounded prompt assembly from configurable TABLE SCHEMES, Gemini-generated BigQuery Standard SQL, async execution via the BigQuery client (with optional dataset location pinning), and structured narrative answers (Direct answer, Key numbers, Insights, Chart hint) tied to actual row previews.",
      "Delivered multi-turn “Data + SQL” chat: conversation transcript passed into SQL generation and answer synthesis (with truncation windows) so follow-ups resolve references like “same player” or “narrow that down”; separate pure chat mode over message history.",
      "Shipped an intent-based query router (classify then dispatch to stat SQL, comparison, prediction narrative, report generation, debate settlement, or open chat) exposed via advanced API, plus a lightweight RAG path: keyword retrieval over curated schema + cricket domain snippets, then Gemini answers with explicit grounding instructions.",
      "Integrated BigQuery ML workflows: train/serve boosted-tree match-outcome models, K-Means player archetyping, and ARIMA-style score forecasting hooks; combined heuristic analytics from SQL (e.g., match and player-form signals) with Gemini explanations for prediction UX.",
      "Productized analytics modules as REST APIs and UI flows: dashboard aggregates, player profiles and search, compare (radar + head-to-head), matches and venues, bowling phase/matchup views, Test partnerships, and debate settler; optional subscription tiers, CORS, and middleware hooks for API key auth and in-memory rate limiting.",
      "Implemented AI report studio: Gemini-generated match and player scout reports from live stats; WeasyPrint pipeline to render Markdown → HTML → PDF, with shared PDF/Markdown (and CSV where tabular) export patterns across reports, predictions, compare, venues, bowling, partnerships, and AI query threads.",
    ],
  },
];
